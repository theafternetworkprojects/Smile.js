var Chess = loadModule("chess.js")
var mcts = new MCTS(Chess())
function MCTS(game){
  this.real_game = game;
  // this.board = board;
...
var generateBestMove = function (move = false) {
    var bestMove = mcts.get_best_move()
    if (move){
      mcts.real_game.move(bestMove.bestMove)
    };
    return bestMove
};
var updateBoard = function (fen){
  mcts.real_game.load(fen)
}
var updateBoardPGN = function (fen){
  mcts.real_game.load_pgn(fen)
}
var move = (data)=>{
  mcts.real_game.move(data)
}
var getBoard = ()=>{return mcts.real_game}
module.exports = {
  getBoard,
  generateBestMove,
  updateBoard,
