...
is_touch_device()?document.addEventListener("touchstart",(event)=>{
    ...
    const x = event.targetTouches[0].offsetX - target.getBoundingoffsetRect().left;
    const y = event.targetTouches[0].offsetY - target.getBoundingoffsetRect().top;
    ...
    const x = event.offsetX - target.getBoundingoffsetRect().left;
    const y = event.offsetY - target.getBoundingoffsetRect().top;
    const maxW = Math.max(x, target.offsetWidth - x);
    const maxH = Math.max(y, target.offsetHeight - y);
    const size = Math.sqrt(maxW * maxW + maxH * maxH);
